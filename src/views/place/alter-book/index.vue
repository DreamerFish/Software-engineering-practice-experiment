    ...
    <a-spin :loading="loading" style="width: 100%">
      <a-card class="general-card">
        ...
        <div class="wrapper">
          ...
          <keep-alive>
            <BaseInfo v-if="step === 1" @change-step="changeStep" :sId="((route.query.rsId) as string)" />
            <ChannelInfo v-else-if="step === 2" @change-step="changeStep" :rId="((route.query.rId) as string)"
              :sId="((route.query.rsId) as string)" />
...
import { updateReservation } from '@/network/module/place';
...
const submitForm = async () => {
  setLoading(true);
  try {
    const { code, message } = await updateReservation(submitModel.value);
    if (code === 20000) {
      step.value = 3;
